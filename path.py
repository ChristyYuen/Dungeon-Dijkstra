...
def dijkstras_shortest_path_to_all(initial_position, graph, adj):
    ...
    """
    frontier = []   # the queue that holds it all
    destination = {}   # every dest possible will be stored here
    coming_from = {}   # initial_positions

    destination[initial_position] = 0
    heappush(frontier, (0, initial_position))   # starting the heap queue

    while len(frontier) is not 0:
        curr_cost, curr_cell = heappop(frontier)
        # print('curr_cost ' + str(curr_cost))
        # print('curr_cell ' + str(curr_cell))

        for neighbor, neighbor_cost in adj(graph, curr_cell):
            # print('neighbor ' + str(neighbor))
            # print('neighbor_cost ' + str(neighbor_cost))
            new_pos = destination[curr_cell] + neighbor_cost
            # print('new_pos ' + str(new_pos))

            if neighbor not in destination or new_pos < destination[neighbor]: # update variables when statement is true
                destination[neighbor] = new_pos
                coming_from[neighbor] = curr_cell
                heappush(frontier, (new_pos, neighbor))
                # print('destination[neighbor] ' + str(destination[neighbor]))
                # print('coming_from[neighbor] ' + str(coming_from[neighbor]))
                # print('frontier ' + str(list(frontier)))
    return destination   # I don't I need to reverse it right?
...
def navigation_edges(level, cell):
    ...
    if cell in level['spaces'].keys():
        ...
        # print('ogCellCost ' + str(ogCellCost))
    ...
    for dir in dirs:
        ...
        if neighbor in level['spaces']:
            ...
            # print('neighborCost ' + str(neighborCost))
        ...
        if neighborCost and ogCellCost is not None:
            ...
            # print('neighborEdgeCost ' + str(neighborEdgeCost))
...
def cost_to_all_cells(filename: object, src_waypoint: object, output_filename: object) -> object:
